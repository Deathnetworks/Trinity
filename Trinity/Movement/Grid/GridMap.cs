...
namespace Trinity
{
    class GridMap
    {
        ...
        public static GridNode GetBestMoveNode(float _minRange = 6f, float _maxRange = 100f, Vector3 _loc = new Vector3(), bool _prioritizeDist = false)
        {
            ...
            var _results = (from p in MainGrid.MapAsList
                 where p.Position.Distance2D(_loc) >= _minRange && p.Position.Distance2D(_loc) < _maxRange
                 select p) .ToList();

            if (_prioritizeDist)
                _results = (from p in _results
                    orderby p.Distance
                    orderby p.SafeWeight descending
                    select p).ToList();
            else
                _results = (from p in _results
                    orderby p.SafeWeight descending
        ...
        public static void RefreshGridMainLoop()
        {
            ...
            string _taskTimers = " /";
            for (int t = 1; t < MainGrid.Timers.Length; t++)
            {
                if (MainGrid.Timers[t].IsRunning) 
                    MainGrid.Timers[t].Stop();
                if (MainGrid.Timers[t].ElapsedMilliseconds >= 1)
                    _taskTimers += string.Format("/ {0}/{1:00} ", t, MainGrid.Timers[t].ElapsedMilliseconds);

                MainGrid.Timers[t].Reset();
            }

            if (_taskTimers != " /" || MainGrid.Timers[0].ElapsedMilliseconds >= 1)
                Logger.Log(TrinityLogLevel.Info, LogCategory.Performance, "Refresh grid took {0}ms{1}", MainGrid.Timers[0].ElapsedMilliseconds, _taskTimers != " /" ? _taskTimers : "");

            MainGrid.Timers[0].Reset();
        }
    }
}
