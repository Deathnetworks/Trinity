...
namespace Trinity.Items
{
    public static class Legendary
    {
        ...
        public static Item MaskofJeramSet = new Item(369016, "Mask of Jeram", ItemType.Helm);
        ...
        public static Item KredesFlame = new Item(197836, "Krede’s Flame", ItemType.Ring);
        ...
        public static Set ThousandStorms = new Set
        {
            ...
            {
                MaskoftheSearingSky,
                ScalesoftheDancingSerpent,
                MantleoftheUpsideDownSinners,
                HeartoftheCrashingWave,
                FistsofThunder,
                EightDemonBoots
            },
        ...
        //    Items = new List<Item>
        //    {
        //        JadeHarvestersCourage,
        //        JadeHarvestersWisdom,
        //        JadeHarvestersJoy
        ...
        public static Set VyrsAmazingArcana = new Set
        {
            Items = new List<Item>
            {
                VyrsAstonishingAura,
                VyrsFantasticFinery,
                VyrsGraspingGauntlets,
                VyrsSwaggeringStance
            },
            FirstBonusItemCount = 2,
            SecondBonusItemCount = 4
        };
        ...
        public static Set CaptainCrimsonTrimmings = new Set
        {
            Items = new List<Item>
            {
                CaptainCrimsonsSilkGirdle,
                CaptainCrimsonsThrust,
                CaptainCrimsonsWaders
            },
            FirstBonusItemCount = 2,
            SecondBonusItemCount = 3
        };
        ...
        public static Set BlackthornesBattlegear = new Set
        {
            Items = new List<Item>
            {
                BlackthornesDuncraigCross,
                BlackthornesJoustingMail,
                BlackthornesNotchedBelt,
                BlackthornesSpurs,
                BlackthornesSurcoat
            },
            FirstBonusItemCount = 2,
        ...
        public static Set TalRashasElements = new Set
        {
            Items = new List<Item>
            {
                TalRashasAllegiance,
                TalRashasBrace,
                TalRashasGuiseofWisdom,
                TalRashasRelentlessPursuit,
                TalRashasUnwaveringGlare
            },
            FirstBonusItemCount = 2,
            SecondBonusItemCount = 3,
            ThirdBonusItemCount = 4
    ...
    public class Equipped
    {
        ...
        private static List<ACDItem> _equippedItems = new List<ACDItem>();

        public static List<ACDItem> EquippedItems
        {
            get
            {
                if (DateTime.UtcNow.Subtract(_lastUpdate) > TimeSpan.FromSeconds(10)) Update();
                return _equippedItems;
            }
        }

        private static HashSet<int> _equippedItemIds = new HashSet<int>();

        public static HashSet<int> EquippedItemIds
        {
            get
            {
                if (DateTime.UtcNow.Subtract(_lastUpdate) > TimeSpan.FromSeconds(10)) Update();
        ...
        {
            if (!ZetaDia.IsInGame || !ZetaDia.Me.IsValid) return;
            _lastUpdate = DateTime.UtcNow;
            _equippedItems = ZetaDia.Me.Inventory.Equipped.ToList();
            _equippedItemIds = new HashSet<int>(_equippedItems.Select(i => i.ActorSNO));
        }
    }
    ...
    public class Item
    {
        ...
        {
            get { return Equipped.EquippedItems.Any(i => i.ActorSNO == Id); }
        }
        ...
        public bool IsBuffActive
        {
            get
            {
                return Lookup.ItemAndSnoPower.ContainsKey(this) && CombatBase.GetHasBuff(Lookup.ItemAndSnoPower[this]);                 
            }
        }

        /// <summary>
        /// Call this from somewhere that updates frequently (like within a combat profile)
        /// It will spam the console when buffs are active and give you their IDs. 
        /// Use to build Lookup.ItemAndSnoPower dictionary.
        /// </summary>
        public static void DebugDumpAllActiveBuffs()
        {
            Enum.GetValues(typeof (SNOPower)).Cast<SNOPower>().ToList().ForEach(p =>
            {
                if (CombatBase.GetHasBuff(p))
                {
                    Logger.Log("Buff {0} is Active", Enum.GetName(typeof (SNOPower), p));
                }
            });
        }
    ...
    public class Lookup
    {
        public readonly static Dictionary<Item, SNOPower> ItemAndSnoPower = new Dictionary<Item, SNOPower>
        {
    ...
    public class Set
    {
        ...
        private readonly Dictionary<SetBonusTier, bool> _bonuses = new Dictionary<SetBonusTier, bool>();

        public bool IsFirstBonusActive
        {
            get { return IsBonusActive(SetBonusTier.First); }
        }

        public bool IsSecondBonusActive
        {
            get { return IsBonusActive(SetBonusTier.Second); }
        }

        public bool IsThirdBonusActive
        {
            get { return IsBonusActive(SetBonusTier.Third); }
        }
        ...
        public bool IsBonusActive(SetBonusTier setBonusTier)
        {
            var equippedPieces = Equipped.EquippedItems.Count(i => ItemIds.Contains(i.ActorSNO));
            var hasRRog = Legendary.RingofRoyalGrandeur.IsEquipped;

            _bonuses[SetBonusTier.First] = (equippedPieces >= FirstBonusItemCount || hasRRog && equippedPieces >= FirstBonusItemCount - 1);
            _bonuses[SetBonusTier.Second] = (equippedPieces >= SecondBonusItemCount || hasRRog && equippedPieces >= SecondBonusItemCount - 1);
            _bonuses[SetBonusTier.Third] = (equippedPieces >= ThirdBonusItemCount || hasRRog && equippedPieces >= ThirdBonusItemCount - 1);
            _bonuses[SetBonusTier.None] = !_bonuses[SetBonusTier.First] && !_bonuses[SetBonusTier.Second] && !_bonuses[SetBonusTier.Third];

            return _bonuses[setBonusTier];
        }
